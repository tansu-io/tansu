...
use tansu_sans_io::{
    ApiKey, Body, ErrorCode, ProduceRequest, ProduceResponse,
...
impl<S> ApiKey for ProduceService<S> {
    const KEY: i16 = ProduceRequest::KEY;
}

impl<S> ProduceService<S>
where
    S: Storage,
{
...
impl<S, State, Q> Service<State, Q> for ProduceService<S>
where
    S: Storage,
    ...
    Q: Into<Body> + Send + Sync + 'static,
{
    ...
    async fn serve(&self, _ctx: Context<State>, request: Q) -> Result<Self::Response, Self::Error> {
        let produce = ProduceRequest::try_from(request.into())?;
        ...
        if let Some(topics) = produce.topic_data {
            for topic in topics {
                debug!(?topic);

                responses.push(self.topic(produce.transactional_id.as_deref(), topic).await)
            }
        }
...
mod tests {
    ...
    use rama::Context;
    use tansu_sans_io::{
        ErrorCode, InitProducerIdRequest, InitProducerIdResponse,
        record::{
            Record,
            deflated::{self, Frame},
            inflated,
    ...
    async fn non_txn_idempotent_unknown_producer_id() -> Result<()> {
        ...
        let service = ProduceService::new(storage);
    ...
    async fn non_txn_idempotent() -> Result<()> {
        ...
        let producer = InitProducerIdService::new(storage.clone())
            .serve(
                Context::default(),
            ...
            .await
            .and_then(|body| InitProducerIdResponse::try_from(body).map_err(Into::into))?;

        let request = ProduceService::new(storage.clone());
