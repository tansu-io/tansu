...
create or replace view v_watermark as
select
c.name as cluster,
t.name as topic,
...
create or replace view v_producer_epoch as
select
c.name as cluster,
p.id as producer_id,
...
create or replace view v_producer_detail as
select
c.name as cluster,
p.id as producer_id,
pe.epoch as producer_epoch,
...
create or replace view v_txn_produce_offset as
select
c.name as cluster,
p.id producer_id,
pe.epoch as producer_epoch,
...
create or replace view v_txn_consumer_offset_tp as
select
c.name as cluster,
p.id as producer_id,
pe.epoch as producer_epoch,
